...
var HeadroomClient = class {
	...
	*/
	async compress(messages, model = "deepseek-chat", mode = "ccr") {
		const body = {
			messages,
			model,
			config: { mode }
...
async function executeHeadroomCommand(ctx, scope, ns, command) {
	try {
	...
	ctx.effect(() => {
		...
		const restart = () => {
			queue = queue.then(async () => {
				...
				if (config.prewarm !== false && started.client !== void 0) started.client.compress([{
					role: "user",
...
function assertValidEngineConfig(config) {
	const ratio = (name$1, value) => {
		if (value < 0 || value > 1) throw new Error(`BasicCompactionConfig: ${name$1} (${value}) must be between 0 and 1`);
	};
	const numberOrThrow = (name$1, value) => {
		if (value === void 0) return void 0;
		if (typeof value !== "number" || !Number.isFinite(value)) throw new Error(`BasicCompactionConfig: ${name$1} must be a finite number`);
		return value;
	};
...
//#endregion
export { Config, HEADROOM_SETTINGS_NS, apply, assertValidEngineConfig, inject, name, restoreCompactionEntries, setCompactionEntries };