...
function installTakeoverRollback(ctx) {
	ctx.effect(() => {
		...
		return async () => {
			...
			ctx.logger.warn("dsh-headroom: could not re-enable compaction-basic on unload; remove its `disabled: true` marker in the loader tree or restart dsh web");
		};
...
//#endregion
export { Config, HEADROOM_SETTINGS_NS, apply, inject, name };