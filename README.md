...
2. **构建用 DSH 源码**:`~/.dsh/source/current` 是指向含 `lib/` 产物的 DSH 检出目录的 junction/symlink(开发依赖经它解析)。本机示例:
...
```powershell
# 1. 克隆(建议放到 ~/.dsh/plugins/ 下,相对 link 路径按此解析)
git clone https://github.com/<you>/dsh-headroom.git "$env:USERPROFILE\.dsh\plugins\dsh-headroom"

# 2. 安装依赖并构建(构建产物 lib/ 已入库,此步可跳过)
cd "$env:USERPROFILE\.dsh\plugins\dsh-headroom"
pnpm install
pnpm build

# 3. 装配(本插件是普通 cordis 插件,只能走 cordis.patch.yml 的 insert)
